...
def makeshape():
	...
	ptsmid[:,1]-=vecs[1]
	#---mesh the points
	monolayer_mesh = makemesh(ptsmid,vecs)
	...
	with open(wordspace['step']+name,'w') as fp:
		...
		for mn in range(2):
			...
			for pnum,p in enumerate(ptsmid):
				...
				xys = p+offset+[1,-1][mn]*dot(rotation_matrix(cross(zvec,monolayer_mesh['vertnorms'][pnum]),dot(zvec,monolayer_mesh['vertnorms'][pnum])),lpts.T).T
				fp.write('\n'.join([''.join([
					str(resnr).rjust(5),
					resname.ljust(5),
					atomnames[i].rjust(5),
